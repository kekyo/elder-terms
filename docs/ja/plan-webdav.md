...
状態: 計画案（対応範囲は確定、実装未着手）。今回の成果物はこの文書であり、製品コードは未変更。各段階の完了条件は実装時に検証する。
...
計画更新時の照合: 上記の要求を設定、共通UI、プロトコル、証明書手順、各実装段階と検証へ割り当てた。一般的なWebDAVに限定する利用者の指定を反映し、特殊機能への対応や接続先製品の指定を待つ条件を除いた。実装の完了条件は未実施であり、今回の文書変更はビルドへ影響しないため、ビルド・テストは実行していない。
