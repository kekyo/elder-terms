...
見つかったホストは探索中でも順次表示され、取得出来た場合は逆引きDNS名、そして開いているポートのカンマ区切り一覧も表示されます。プログレスバーは、統合された探索範囲の確認状況を示します。行をダブルクリックすると探索を中断してダイアログを閉じ、その数値IPアドレスを設定へ入力します。「キャンセル」をクリックした場合は、探索を中断してダイアログを閉じますが、アドレス設定は変更しません。
