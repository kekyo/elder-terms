...
見つかったホストは探索中でも順次表示され、取得出来た場合は逆引きDNS名も表示されます。「SSH/SFTP(22)」「TELNET(23)」「FTP(21)」の各列には、対応するポートが見つかった場合にチェックマークが表示されます。プログレスバーは、統合された探索範囲の確認状況を示します。行をダブルクリックすると探索を中断してダイアログを閉じ、その時点で取得済みの名前を設定へ入力します。名前がない場合は数値IPアドレスを入力します。接続種別、ポート、接続エントリ名は変更しません。「キャンセル」をクリックした場合は、探索を中断してダイアログを閉じますが、アドレス設定は変更しません。

探索対象は、このマシンに設定されたIPv4範囲だけです。IPv4アドレスがないインターフェイスは無視されます。ループバックインターフェイスでは、ループバック範囲全体ではなく、割り当て済みのアドレスだけを探索します。DNS逆引きの応答が遅い場合は、完了まで時間がかかることがあります。
