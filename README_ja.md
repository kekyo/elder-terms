...
TELNET、SSH/SFTP、FTPの設定では、アドレス入力欄の右側に「IPスキャン」ボタンがあります。開くとすぐに、IPv4アドレスが設定されたすべてのネットワークインターフェイスについて、FTP（21）、SSH/SFTP（22）、TELNET（23）の標準TCPポートを探索します。例えば `/24` のインターフェイスでは、末尾が0から255までの全アドレスが対象です。
...
探索対象は、このマシンに設定されたIPv4範囲だけです。IPv4アドレスがないインターフェイスは無視されます。広い範囲や応答が遅いDNS逆引きでは、完了まで時間がかかる場合があります。
