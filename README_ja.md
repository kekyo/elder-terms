...
ランチャーのアプリケーションメニューから「アプリケーション設定」を開き、「リンク」ページを選択すると、リンクアクション全体の有効・無効、ルールの追加・削除・並べ替え、コマンド引数の編集、組み込み既定値への復元を行えます。設定はすべての接続に適用されます。

リンク設定を明示していない場合は、次の組み込みルールが使用されます。
...
キャプチャに含まれる`%20`などのURIエスケープは、コマンドへ渡す前にデコード出来ます。「既定値に戻す」を実行すると明示的なリンク設定が削除されるため、将来組み込みの既定値が変更された場合も反映されます。
...
同じ設定を`~/.config/elder-terms/global.ini`で編集することも出来ます。次の例は、絶対ローカルソースパスとそれに続く行番号を認識します。
...
INIファイルでリンク設定を一つでも明示すると、すべての組み込みルールがカスタムルールで置き換えられます。すべて無効にする場合は、`[hyperlink]`の`enabled=false`を指定します。この設定はグローバル設定専用であり、接続設定や一時的な起動プロファイルにあるリンクセクションは無視されます。
