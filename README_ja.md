...
見つかったホストは探索中でも順次表示され、取得出来た場合は逆引きDNS名も表示されます。「SSH/SFTP(22)」「TELNET(23)」「FTP(21)」の各列には、対応するポートが見つかった場合にチェックマークが表示されます。プログレスバーは、統合された探索範囲の確認状況を示します。行をダブルクリックすると探索を中断してダイアログを閉じ、その数値IPアドレスを設定へ入力します。「キャンセル」をクリックした場合は、探索を中断してダイアログを閉じますが、アドレス設定は変更しません。
