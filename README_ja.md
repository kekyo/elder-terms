...
別の接続を作る場合は、ランチャー下部の "New" ボタンをクリックして、名前を "New connection" から適当な名前に変更し、"Save" をクリックします。
ほら:
...
開くとすぐに、IPv4アドレスが設定されたすべてのネットワークインターフェイスについて、FTP（21）、SSH/SFTP（22）、TELNET（23）の標準TCPポートを探索します。
...
切断後もターミナルを残すには、一般設定の「セッション終了時にウィンドウを閉じる」を無効にして下さい（`[general] auto_close=false`）。SSHとTELNETでは切断オーバーレイに「再接続」ボタンが表示され、前の接続の処理が終了するとクリック出来るようになります。

同じ設定はFTP・FTPS・SFTP・WebDAVのウィンドウにも適用され、接続先の切断を検出した際にウィンドウを閉じるかどうかを指定します。
...
Basic/Digest認証では、ユーザー名とパスワードを別々のパネルで順に入力します。パスワードは保存しません。
...
SFTP/FTP/FTPS/WebDAVは、同じ2ペインのファイルブラウザを使用します。ローカルまたはリモートペーンで項目を選択して右クリックすると、項目メニューが開きます。メニューを開く前に、`Ctrl`+クリックまたは矩形ドラッグで複数の項目を選択出来ます。
...
「端末」タブを下へスクロールすると、優先順位付きのフォントファミリー一覧を編集出来ます。行を追加・削除し、上下ボタンで順序を変更出来ます。各行には名前を直接入力するか、フォント選択ダイアログから指定します。
