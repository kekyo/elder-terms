...
static bool handshake(elder_terms::TlsCertificatePolicy &policy,
                      const std::string &certificate, const std::string &key,
                      bool trusted, std::size_t identity_slot) {
  policy.failure.reset();
  policy.callback_failure = nullptr;
  policy.identity_slot = identity_slot;
  ...
  expect(elder_terms::configure_tls_certificate_context(nullptr, client_context.get(), &policy) == CURLE_OK,
...
int main(int argc, char **argv) {
  try {
    ...
    auto policy = elder_terms::create_tls_certificate_policy(connection.address, connection.port,
        connection.tls_mode == elder_terms::FtpTlsMode::implicit_tls ? "ftps" : "ftp", 2);
    const std::string result = argv[5];
    const bool trusted = std::string(argv[4]) == "trusted";
    using elder_terms::ftp_control_certificate_identity;
    using elder_terms::ftp_data_certificate_identity;
    const bool accepted = handshake(*policy, argv[2], argv[3], trusted, ftp_control_certificate_identity);
    if (result == "success" || result == "failure") {
      ...
      elder_terms::approve_tls_certificate_failure(*policy, first);
      expect(handshake(*policy, argv[2], argv[3], false, ftp_control_certificate_identity), "Same control certificate must retain its approval");
      expect(handshake(*policy, argv[2], argv[3], false, ftp_data_certificate_identity), "Same data certificate may use the control approval");
      expect(!handshake(*policy, result, argv[3], false, ftp_data_certificate_identity), "Changed data certificate must prompt");
      expect(policy->failure->sha256 != first.sha256, "Changed certificate must have a distinct identity");
      elder_terms::approve_tls_certificate_failure(*policy, *policy->failure);
      expect(handshake(*policy, result, argv[3], false, ftp_data_certificate_identity), "Approved replacement must work");
      expect(!handshake(*policy, argv[2], argv[3], false, ftp_data_certificate_identity), "Returning to an older certificate must prompt again");
      auto another = elder_terms::create_tls_certificate_policy(connection.address, connection.port,
        connection.tls_mode == elder_terms::FtpTlsMode::implicit_tls ? "ftps" : "ftp", 2);
      expect(!handshake(*another, argv[2], argv[3], false, ftp_control_certificate_identity), "A new session must not inherit approval");
    }
